...
#include <stdio.h>
#include <math.h>
#include <stdint.h>    // Added for standard integer types
#include <stdlib.h>
#include <string.h>    // for memset
#include <stddef.h>    // for NULL

// Include the pico stdlib at the top of the file
#include "pico/stdlib.h"
#include "WPCS6809.h"

// --- MAME Standalone Compatibility Block ---
...
typedef struct
{
	...
	UINT32		timer_A_accum;
	UINT32		timer_B_accum;
	UINT32		timer_A_divider;
	UINT32		timer_B_divider;
...
#define TL_TAB_MAX_SAFE 16384
static __not_in_flash("tl_tab") signed int tl_tab[TL_TAB_MAX_SAFE];

#define ENV_QUIET		(TL_TAB_LEN>>3)

/* sin waveform table in 'decibel' scale */
static __not_in_flash("sin_tab") unsigned int sin_tab[SIN_LEN];

#define RATE_STEPS (8)
static __not_in_flash("eg_inc") UINT8 eg_inc[19*RATE_STEPS]={
...
static __not_in_flash("eg_rate_select") UINT8 eg_rate_select[32+64+32]={	/* Envelope Generator rates (32 + 64 rates + 32 RKS) */
...
#define O(a) (a*1)
// Remove const and add RAM attribute
static __not_in_flash("eg_rate_shift") UINT8 eg_rate_shift[32+64+32]={	/* Envelope Generator counter shifts (32 + 64 rates + 32 RKS) */
...
*/
// Remove const and add RAM attribute
static __not_in_flash("dt2_tab") UINT32 dt2_tab[4] = { 0, 384, 500, 608 };
...
// Remove const and add RAM attribute
static __not_in_flash("dt1_tab") UINT8 dt1_tab[4*32] = { /* 4*32 DT1 values */
/* DT1=0 */
...
static __not_in_flash("phaseinc_rom") const UINT16 phaseinc_rom[768] = {
...
// Remove const and add RAM attribute
static __not_in_flash("lfo_noise") UINT8 lfo_noise_waveform[256] = {
...
static void init_tables(void)
{
	...
	for (i = TL_TAB_LEN; i < TL_TAB_MAX_SAFE; i++)
	{
		tl_tab[i] = 0;
	}	 
...
static void init_chip_tables()
{
	...
	scaler = ( (double)g_ym_chip.clock / 64.0 ) / ( (double)g_ym_chip.sampfreq );
	...
	for (i=0; i<768; i++)
	{
		...
		g_ym_chip.freq[ 768+2*768+i ] = ((int)(phaseinc*mult)) & 0xffffffc0; /* adjust to X.10 fixed point */
		/* octave 0 and octave 1 */
		for (j=0; j<2; j++)
		{
			g_ym_chip.freq[768 + j*768 + i] = (g_ym_chip.freq[ 768+2*768+i ] >> (2-j) ) & 0xffffffc0; /* adjust to X.10 fixed point */
		}
		/* octave 3 to 7 */
		for (j=3; j<8; j++)
		{
			g_ym_chip.freq[768 + j*768 + i] = g_ym_chip.freq[ 768+2*768+i ] << (j-2);
		}
	...
	for (i=0; i<768; i++)
	{
		g_ym_chip.freq[ 0*768 + i ] = g_ym_chip.freq[1*768+0];
	}
	...
	for (j=8; j<10; j++)
	{
		for (i=0; i<768; i++)
		{
			g_ym_chip.freq[768+ j*768 + i ] = g_ym_chip.freq[768 + 8*768 -1];
		}
	}
	...
	for (j=0; j<4; j++)
	{
		for (i=0; i<32; i++)
		{
			Hz = ( (double)dt1_tab[j*32+i] * ((double)g_ym_chip.clock/64.0) ) / (double)(1<<20);

			/*calculate phase increment*/
			phaseinc = (Hz*SIN_LEN) / (double)g_ym_chip.sampfreq;

			/*positive and negative values*/
			g_ym_chip.dt1_freq[ (j+0)*32 + i ] = phaseinc * mult;
			g_ym_chip.dt1_freq[ (j+4)*32 + i ] = -g_ym_chip.dt1_freq[ (j+0)*32 + i ];
	...
	for (i=0; i<32; i++)
	{
		...
		g_ym_chip.noise_tab[i] = j * 64 * scaler;
	}
}
...
#define KEY_ON(op, key_set){									\
		...
		{														\
			...
			(op)->volume += (~(op)->volume *					\
                           (eg_inc[(op)->eg_sel_ar + ((g_ym_chip.eg_cnt>>(op)->eg_sh_ar)&7)])	\
                          ) >>4;								\
...
void ym2151_write_reg(int r, int v)
{
	YM2151Operator *op = &g_ym_chip.oper[ (r&0x07)*4+((r&0x18)>>3) ];
	...
	case 0x00:
		...
		case 0x01:	/* LFO reset(bit 1), Test Register (other bits) */
			g_ym_chip.test = v;
			if (v&2) g_ym_chip.lfo_phase = 0;
			break;

		case 0x08:
			//PSG = chip; /* PSG is used in KEY_ON macro */
			envelope_KONKOFF(&g_ym_chip.oper[ (v&7)*4 ], v );
			break;

		case 0x0f:	/* noise mode enable, noise period */
			g_ym_chip.noise = v;
			g_ym_chip.noise_f = g_ym_chip.noise_tab[ v & 0x1f ];
			break;

		case 0x10:	/* timer A hi */
			g_ym_chip.timer_A_index = (g_ym_chip.timer_A_index & 0x003) | (v<<2);
			break;

		case 0x11:	/* timer A low */
			g_ym_chip.timer_A_index = (g_ym_chip.timer_A_index & 0x3fc) | (v & 3);
			break;

		case 0x12:	/* timer B */
			g_ym_chip.timer_B_index = v;
			break;

		case 0x14:	/* CSM, irq flag reset, irq enable, timer start/stop */

			g_ym_chip.irq_enable = v;	/* bit 3-timer B, bit 2-timer A, bit 7 - CSM */

			if (v&0x10)	/* reset timer A irq flag */
			{
				int oldstate = g_ym_chip.status & 3;
				g_ym_chip.status &= ~1;
				gpio_put(24, 0);
			}
			...
			if (v&0x02){	/* load and start timer B */
				...
				{
					g_ym_chip.tim_B = 1;
					g_ym_chip.timer_B_accum = g_ym_chip.timer_B_index;
				}
			}else{		/* stop timer B */
				#ifdef USE_MAME_TIMERS
				/* ASG 980324: added a real timer */
					timer_enable(g_ym_chip.timer_B, 0);
				#else
					g_ym_chip.tim_B = 0;
				#endif
			}
			...
			if (v&0x01){	/* load and start timer A */
				...
				{
					g_ym_chip.tim_A = 1;
					g_ym_chip.timer_A_accum = g_ym_chip.timer_A_index;
				}
			}else{		/* stop timer A */
				g_ym_chip.tim_A = 0;
			}
		...
		case 0x18:	/* LFO frequency */
			{
				g_ym_chip.lfo_overflow    = ( 1 << ((15-(v>>4))+3) ) * (1<<LFO_SH);
				g_ym_chip.lfo_counter_add = 0x10 + (v & 0x0f);
			}
			break;

		case 0x19:	/* PMD (bit 7==1) or AMD (bit 7==0) */
			if (v&0x80)
				g_ym_chip.pmd = v & 0x7f;
			else
				g_ym_chip.amd = v & 0x7f;
			break;

		case 0x1b:	/* CT2, CT1, LFO waveform */
			g_ym_chip.ct = v >> 6;
			g_ym_chip.lfo_wsel = v & 3;
			break;
	...
	case 0x20:
		op = &g_ym_chip.oper[ (r&7) * 4 ];
		switch(r & 0x18){
		case 0x00:	/* RL enable, Feedback, Connection */
			op->fb_shift = ((v>>3)&7) ? ((v>>3)&7)+6:0;
			g_ym_chip.pan[ (r&7)*2    ] = (v & 0x40) ? ~0 : 0;
			g_ym_chip.pan[ (r&7)*2 +1 ] = (v & 0x80) ? ~0 : 0;
			g_ym_chip.connect[r&7] = v&7;
		...
		case 0x08:	/* Key Code */
			...
			if (v != op->kc)
			{
				...
				(op+0)->dt1 = g_ym_chip.dt1_freq[ (op+0)->dt1_i + kc ];
				(op+0)->freq = ( (g_ym_chip.freq[ kc_channel + (op+0)->dt2 ] + (op+0)->dt1) * (op+0)->mul ) >> 1;

				(op+1)->dt1 = g_ym_chip.dt1_freq[ (op+1)->dt1_i + kc ];
				(op+1)->freq = ( (g_ym_chip.freq[ kc_channel + (op+1)->dt2 ] + (op+1)->dt1) * (op+1)->mul ) >> 1;

				(op+2)->dt1 = g_ym_chip.dt1_freq[ (op+2)->dt1_i + kc ];
				(op+2)->freq = ( (g_ym_chip.freq[ kc_channel + (op+2)->dt2 ] + (op+2)->dt1) * (op+2)->mul ) >> 1;

				(op+3)->dt1 = g_ym_chip.dt1_freq[ (op+3)->dt1_i + kc ];
				(op+3)->freq = ( (g_ym_chip.freq[ kc_channel + (op+3)->dt2 ] + (op+3)->dt1) * (op+3)->mul ) >> 1;
	...
	case 0x40:		/* DT1, MUL */
		{
			...
			if (olddt1_i != op->dt1_i)
				op->dt1 = g_ym_chip.dt1_freq[ op->dt1_i + (op->kc>>2) ];

			if ( (olddt1_i != op->dt1_i) || (oldmul != op->mul) )
				op->freq = ( (g_ym_chip.freq[ op->kc_i + op->dt2 ] + op->dt1) * op->mul ) >> 1;
		}
		break;
...
int ym2151_read_status()
{
	return g_ym_chip.status;
}
...
void ym2151_init(int clock, int rate)
{
	...
	memset(&g_ym_chip, 0, sizeof(YM2151));

	init_tables();

	g_ym_chip.clock = clock;
	g_ym_chip.sampfreq = rate ? rate : 44100;	/* avoid division by 0 in init_chip_tables() */
	init_chip_tables();

	g_ym_chip.lfo_timer_add = (1<<LFO_SH) * (clock/64.0) / g_ym_chip.sampfreq;
	g_ym_chip.eg_timer_add  = (1<<EG_SH)  * (clock/64.0) / g_ym_chip.sampfreq;
	g_ym_chip.eg_timer_overflow = ( 3 ) * (1<<EG_SH);
	g_ym_chip.tim_A      = 0;
	g_ym_chip.tim_B      = 0;

	for (chn = 0; chn < 8; chn ++)
		g_ym_chip.Muted[chn] = 0x00;
...
void ym2151_reset_chip()
{
	...
	for (i=0; i<32; i++)
	{
		memset(&g_ym_chip.oper[i],'\0',sizeof(YM2151Operator));
		g_ym_chip.oper[i].volume = MAX_ATT_INDEX;
	        g_ym_chip.oper[i].kc_i = 768; /* min kc_i value */
	}

	g_ym_chip.eg_timer = 0;
	g_ym_chip.eg_cnt   = 0;

	g_ym_chip.lfo_timer  = 0;
	g_ym_chip.lfo_counter= 0;
	g_ym_chip.lfo_phase  = 0;
	g_ym_chip.lfo_wsel   = 0;
	g_ym_chip.pmd = 0;
	g_ym_chip.amd = 0;
	g_ym_chip.lfa = 0;
	g_ym_chip.lfp = 0;
	...
	g_ym_chip.irq_enable = 0;
	g_ym_chip.tim_A      = 0;
	g_ym_chip.tim_B      = 0;
	g_ym_chip.timer_A_index = 0;
	g_ym_chip.timer_B_index = 0;
	g_ym_chip.timer_A_accum = 0;
	g_ym_chip.timer_B_accum = 0;

	g_ym_chip.noise     = 0;
	g_ym_chip.noise_rng = 0;
	g_ym_chip.noise_p   = 0;
	g_ym_chip.noise_f   = g_ym_chip.noise_tab[0];
...
static inline signed int __not_in_flash_func(op_calc)(YM2151Operator * OP, unsigned int env, signed int pm)
{
	UINT32 p;
...
static inline signed int __not_in_flash_func(op_calc1)(YM2151Operator * OP, unsigned int env, signed int pm)
{
	UINT32 p;
	INT32  i;
...
static inline void __not_in_flash_func(chan_calc)(unsigned int chan)
{
	YM2151Operator *op;
	unsigned int env;
	UINT32 AM = 0;

	if (g_ym_chip.Muted[chan])
		return;
	
	m2 = c1 = c2 = mem = 0;
	op = &g_ym_chip.oper[chan*4];	/* M1 */

	*op->mem_connect = op->mem_value;	/* restore delayed sample (MEM) value to m2 or c2 */

	if (op->ams)
		AM = g_ym_chip.lfa << (op->ams-1);
	env = volume_calc(op);
	{
...
static inline void __not_in_flash_func(chan7_calc)(void)
{
	YM2151Operator *op;
	unsigned int env;
	UINT32 AM = 0;

	if (g_ym_chip.Muted[7])
		return;
	
	m2 = c1 = c2 = mem = 0;
	op = &g_ym_chip.oper[7*4];		/* M1 */
	...
	*op->mem_connect = op->mem_value;	/* restore delayed sample (MEM) value to m2 or c2 */
	...
	if (g_ym_chip.noise & 0x80)
	{
		...
		chanout[7] += ((g_ym_chip.noise_rng&0x10000) ? noiseout: -noiseout); /* bit 16 -> output */
	}
	else
	{
		if (env < ENV_QUIET)
...
static inline void __not_in_flash_func(advance_eg)(void)
{
	YM2151Operator *op;
	unsigned int i;

	g_ym_chip.eg_timer += g_ym_chip.eg_timer_add;

	while (g_ym_chip.eg_timer >= g_ym_chip.eg_timer_overflow)
	{
		g_ym_chip.eg_timer -= g_ym_chip.eg_timer_overflow;

		g_ym_chip.eg_cnt++;

		/* envelope generator */
		op = &g_ym_chip.oper[0];	/* CH 0 M1 */
		i = 32;
		do
		{
			switch(op->state)
			...
			case EG_ATT:	/* attack phase */
				if ( !(g_ym_chip.eg_cnt & ((1<<op->eg_sh_ar)-1) ) )
				{
					op->volume += (~op->volume *
                                   (eg_inc[op->eg_sel_ar + ((g_ym_chip.eg_cnt>>op->eg_sh_ar)&7)])
                                  ) >>4;
			...
			case EG_DEC:	/* decay phase */
				if ( !(g_ym_chip.eg_cnt & ((1<<op->eg_sh_d1r)-1) ) )
				{
					op->volume += eg_inc[op->eg_sel_d1r + ((g_ym_chip.eg_cnt>>op->eg_sh_d1r)&7)];
			...
			case EG_SUS:	/* sustain phase */
				if ( !(g_ym_chip.eg_cnt & ((1<<op->eg_sh_d2r)-1) ) )
				{
					op->volume += eg_inc[op->eg_sel_d2r + ((g_ym_chip.eg_cnt>>op->eg_sh_d2r)&7)];
			...
			case EG_REL:	/* release phase */
				if ( !(g_ym_chip.eg_cnt & ((1<<op->eg_sh_rr)-1) ) )
				{
					op->volume += eg_inc[op->eg_sel_rr + ((g_ym_chip.eg_cnt>>op->eg_sh_rr)&7)];
...
static inline void __not_in_flash_func(advance)(void)
{
	YM2151Operator *op;
	unsigned int i;
	int a,p;
	...
	{
		g_ym_chip.lfo_timer += g_ym_chip.lfo_timer_add;
		if (g_ym_chip.lfo_timer >= g_ym_chip.lfo_overflow)
		{
			g_ym_chip.lfo_timer   -= g_ym_chip.lfo_overflow;
			g_ym_chip.lfo_counter += g_ym_chip.lfo_counter_add;
			g_ym_chip.lfo_phase   += (g_ym_chip.lfo_counter>>4);
			g_ym_chip.lfo_phase   &= 255;
			g_ym_chip.lfo_counter &= 15;
		}
	}

	i = g_ym_chip.lfo_phase;
	...
	g_ym_chip.lfa = a * g_ym_chip.amd / 128;
	g_ym_chip.lfp = p * g_ym_chip.pmd / 128;
    ...
    */
	g_ym_chip.noise_p += g_ym_chip.noise_f;
	i = (g_ym_chip.noise_p>>16);		/* number of events (shifts of the shift register) */
	g_ym_chip.noise_p &= 0xffff;
	while (i)
	{
		UINT32 j;
		j = ( (g_ym_chip.noise_rng ^ (g_ym_chip.noise_rng>>3) ) & 1) ^ 1;
		g_ym_chip.noise_rng = (j<<16) | (g_ym_chip.noise_rng>>1);
		i--;
	}
	...
	do
	{
		...
		{
			INT32 mod_ind = g_ym_chip.lfp;		/* -128..+127 (8bits signed) */
			if (op->pms < 6)
				mod_ind >>= (6 - op->pms);
			else
				mod_ind <<= (op->pms - 5);
			...
			if (mod_ind)
			{
				...
				(op+0)->phase += ( (g_ym_chip.freq[ kc_channel + (op+0)->dt2 ] + (op+0)->dt1) * (op+0)->mul ) >> 1;
				(op+1)->phase += ( (g_ym_chip.freq[ kc_channel + (op+1)->dt2 ] + (op+1)->dt1) * (op+1)->mul ) >> 1;
				(op+2)->phase += ( (g_ym_chip.freq[ kc_channel + (op+2)->dt2 ] + (op+2)->dt1) * (op+2)->mul ) >> 1;
				(op+3)->phase += ( (g_ym_chip.freq[ kc_channel + (op+3)->dt2 ] + (op+3)->dt1) * (op+3)->mul ) >> 1;
			}
    ...
    */
	...
	if (g_ym_chip.csm_req)			/* CSM KEYON/KEYOFF seqeunce request */
	{
		if (g_ym_chip.csm_req==2)	/* KEY ON */
		{
			op = &g_ym_chip.oper[0];	/* CH 0 M1 */
			i = 32;
			do
...
uint32_t __not_in_flash_func(ym2151_advance_timers_one_tick)(void) {

    // Only process Timer A if the CPU has started it
    if (g_ym_chip.tim_A) {
        g_ym_chip.timer_A_divider++;
        if (g_ym_chip.timer_A_divider >= 64) {
            g_ym_chip.timer_A_divider = 0;
            g_ym_chip.timer_A_accum++;
            
            if (g_ym_chip.timer_A_accum == 1024) {
                // Reload the counter from the index 
                g_ym_chip.timer_A_accum = g_ym_chip.timer_A_index;
                
                if (g_ym_chip.irq_enable & 0x04) {
                    g_ym_chip.status |= 1;
					gpio_put(24, 1);
					gpio_put(25, 1);
					// flag the CPU for an IRQ
					WPCSCPUFIRQ(true);
                }
                if (g_ym_chip.irq_enable & 0x80) {
                    g_ym_chip.csm_req = 2;  // request KEY ON / KEY OFF sequence
                }
            }
        }
    }

    // Only process Timer B if the CPU has started it
    if (g_ym_chip.tim_B) {
        g_ym_chip.timer_B_divider++;
        if (g_ym_chip.timer_B_divider >= 1024) {
            g_ym_chip.timer_B_divider = 0;
            g_ym_chip.timer_B_accum++;
            
            if (g_ym_chip.timer_B_accum == 256) {
                // Reload the counter from the index
                g_ym_chip.timer_B_accum = g_ym_chip.timer_B_index; 
                
                if (g_ym_chip.irq_enable & 0x08) {
                    g_ym_chip.status |= 2;
                }
            }
        }
...
*/
void __not_in_flash_func(ym2151_update_one)(int32_t *outputL, int32_t *outputR)
{
	int i, chn;
	int32_t outl,outr;
	...
	outl = chanout[0] & g_ym_chip.pan[0];
	outr = chanout[0] & g_ym_chip.pan[1];
	outl += (chanout[1] & g_ym_chip.pan[2]);
	outr += (chanout[1] & g_ym_chip.pan[3]);
	outl += (chanout[2] & g_ym_chip.pan[4]);
	outr += (chanout[2] & g_ym_chip.pan[5]);
	outl += (chanout[3] & g_ym_chip.pan[6]);
	outr += (chanout[3] & g_ym_chip.pan[7]);
	outl += (chanout[4] & g_ym_chip.pan[8]);
	outr += (chanout[4] & g_ym_chip.pan[9]);
	outl += (chanout[5] & g_ym_chip.pan[10]);
	outr += (chanout[5] & g_ym_chip.pan[11]);
	outl += (chanout[6] & g_ym_chip.pan[12]);
	outr += (chanout[6] & g_ym_chip.pan[13]);
	outl += (chanout[7] & g_ym_chip.pan[14]);
	outr += (chanout[7] & g_ym_chip.pan[15]);
...
void ym2151_set_mutemask(UINT32 MuteMask)
{
	UINT8 CurChn;
	
	for (CurChn = 0; CurChn < 8; CurChn ++)
		g_ym_chip.Muted[CurChn] = (MuteMask >> CurChn) & 0x01;
